...
export function getDisplayNextScheduledTradingDate(
  startDate: string,
  frequency: AutoInvestFrequency,
  market: SupportedMarket,
  reference: Date
): string | null {
  const raw = getNextScheduledTradingDate(startDate, frequency, market, reference);
  if (!raw) {
    return null;
  }
  return getDisplayDateForMarket(raw, market);
}
