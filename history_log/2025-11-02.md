...
### 15. 자동 투자 주말 거래 정규화 및 통화 표시 보정 (11/02 추가)
- Firestore에 누적된 자동 투자 거래 중 주말 날짜로 저장된 내역을 조회 시 자동으로 다음 영업일로 이동시키고, 필요 시 환율도 해당 영업일 기준으로 갱신합니다.
- 이 로직은 포트폴리오 전체 조회와 개별 포지션 조회 모두에 적용되어 과거 데이터도 순차적으로 정리됩니다.
- 거래 테이블이 각 거래의 통화 정보를 직접 판별하여 원화 거래는 원화로, 달러 거래는 달러로 정확히 표시하도록 UI를 보완했습니다.

### 17. 거래 이력 액션 정리 및 통화 정규화 심화 (11/02 추가)
- 거래 이력 페이지 헤더에서 `매도 기록`, `내보내기` 버튼을 제거해 실사용 기능만 남겼습니다.
- 모든 거래(자동·수동 포함)에 대해 통화 정보를 재판별하고 필요 시 Firestore에 바로 업데이트합니다.
- USD 거래인데 환율이 비어 있는 경우 히스토리컬 환율을 조회해 저장하도록 보완했습니다.

### 16. 거래 이력 페이지 UI 스타일 개선 (11/02 추가)
- **목표**: 디자인 샘플 기반으로 통일된 스타일 시스템 구축, Variants 패턴으로 재사용 가능한 컴포넌트 관리
- **브랜치**: `feature/transaction-ui-enhancement` (롤백 가능)

**신규 컴포넌트**:
- `components/ui/stat-card.tsx`: 통계 카드 전용 컴포넌트
  - variants: default, buy, sell, summary, neutral
  - 배경색, 테두리, 그림자 효과를 variant별로 차별화
  - StatCardHeader, StatCardTitle, StatCardValue, StatCardDescription, StatCardContent 서브 컴포넌트 제공

**수정된 컴포넌트**:
- `components/ui/card.tsx`: variants 지원 추가
  - variant: default, elevated (hover 시 shadow 강화)
  - 기존 사용처에는 영향 없음 (선택적 prop)
  
- `components/ui/button.tsx`: 새로운 variants 추가
  - `sell`: 매도 버튼용 (빨간색 + 그림자 강화)
  - `export`: 내보내기 버튼용 (outline + 그림자)

**거래 이력 페이지 스타일 적용** (`app/transactions/page.tsx`):
- 페이지 배경: 그라디언트 추가 (`bg-gradient-to-br from-background to-muted/5`)
- 통계 카드: StatCard 컴포넌트로 교체, variant 적용
  - 총 거래 횟수: neutral variant
  - USD/KRW 거래 내역: buy variant (매수는 파란색 강조, 매도는 빨간색 텍스트)
  - 표시 통화 기준 합산: summary variant (amber 색상)
- 액션 버튼: 새 variants 사용
  - 매도 기록: `variant="sell"`
  - 내보내기: `variant="export"`
- 필터 카드: `variant="elevated"` 적용
- 거래 목록 카드: `variant="elevated"` 적용
- 모바일 카드: 좌측 컬러 보더 추가
  - 매수: `border-l-blue-500 bg-blue-50/30`
  - 매도: `border-l-red-500 bg-red-50/30`
- 테이블 헤더: 스타일 강화
  - `font-semibold text-xs uppercase tracking-wider text-muted-foreground`
- 테이블 행: hover 효과 개선 (`hover:bg-muted/70 transition-colors`)

**매도 모달 스타일 개선** (`components/SellStockModal.tsx`):
- DialogContent: 최대 높이 및 스크롤 설정 (`max-h-[90vh] overflow-y-auto`)
- DialogHeader: 제목 크기 증가, 간격 조정
- 입력 필드 그룹: 배경 및 테두리로 시각적 그룹핑 (`p-4 rounded-lg border bg-muted/30`)
- 거래 요약 카드: 스타일 강화
  - 테두리 강화 (`border-2 border-red-200`)
  - 배경색 조정 (`bg-red-50/50`)
  - 아이콘 색상 추가 (`text-red-600`)
  - 제목 크기 증가 (`font-bold text-lg`)
- DialogFooter: 상단 테두리 추가, 버튼 최소 너비 설정
- 매도 기록 버튼: `variant="sell"` 사용

**테스트 결과**:
- ✅ 린트 검사 통과 (우리가 수정한 파일에서 에러 없음)
- ✅ 타입 체크 통과 (우리가 수정한 파일에서 에러 없음)
- ✅ 기존 기능 100% 보존 (onClick, state, API 호출 등 모든 로직 유지)
- ✅ 다크모드 지원 완료
- ✅ 모바일 반응형 유지
